    ...
    def get(self, request):
        ...
        try:
            with connection.cursor() as cur:
                if view == "pergame":
                    cur.execute(
                        "SELECT l.game, l.player_name, MAX(l.score) as score "
                        "FROM leaderboard l JOIN players p ON l.user_id = p.id "
                        "WHERE p.is_admin = 0 GROUP BY l.game, l.player_name ORDER BY l.game, score DESC"
                    )
                    rows = cur.fetchall()
                    cur.execute(
                    ...
                    games: dict = {}
                    for game_name, player_name, score in rows:
                        if game_name not in games:
                            games[game_name] = []
                        if len(games[game_name]) < 3:
                            games[game_name].append({"player_name": player_name, "score": score})
                    return Response({"games": games, "game_list": game_list})

                if view == "game":
                    cur.execute(
                        "SELECT l.player_name, MAX(l.score) as best_score, COUNT(*) as plays, MAX(l.created_at) as last_played "
                        "FROM leaderboard l JOIN players p ON l.user_id = p.id "
                        "WHERE l.game = %s AND p.is_admin = 0 GROUP BY l.player_name ORDER BY best_score DESC LIMIT 10",
                        [game]
                    )
                    rows = cur.fetchall()
                    players = [{"player_name": r[0], "best_score": r[1], "plays": r[2], "last_played": str(r[3])} for r in rows]
                ...
                if view == "rankings":
                    ...
                    cur.execute(
                        ...
                        "COUNT(l.id) AS total_games, COALESCE(AVG(l.score),0) AS avg_score, MAX(l.created_at) AS last_played, p.birthdate "
                        "FROM players p LEFT JOIN leaderboard l ON p.id = l.user_id "
                        "WHERE p.is_admin = 0 "
                        "GROUP BY p.id, p.player_name, p.birthdate HAVING total_points > 0 ORDER BY total_points DESC LIMIT 50"
                    )
                    rows = cur.fetchall()
                ...
                cur.execute(
                    f"SELECT l.player_name, l.user_id, SUM(l.score) as total_score, MAX(l.created_at) as last_played {game_col}, p.birthdate "
                    f"FROM leaderboard l LEFT JOIN players p ON l.user_id = p.id "
                    f"{where} GROUP BY l.player_name, l.user_id, p.birthdate ORDER BY total_score DESC LIMIT 20",
                    params
                )
                rows = cur.fetchall()
                ...
                return Response({"players": players, "game_types": game_types})
